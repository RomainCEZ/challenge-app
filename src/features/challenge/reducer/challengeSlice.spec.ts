...
describe("challenge reducer", () => {
  ...
  it("should handle initial state", () => {
    expect(challengeReducer(undefined, { type: "unknown" })).toEqual({
      challenges: [],
    });
  });
  ...
  it("should set the state", () => {
    const actual = challengeReducer(
      emptyInitialState,
      setChallenges(oneChallengeInitialState.challenges)
    );
  ...
  it("should set activeChallenge to true", () => {
    const actual = challengeReducer(
      oneChallengeInitialState,
      challengeChampion({ id: "1", challenger: "Vador" })
    );
  ...
  it("should increament winstreak", () => {
    const actual = challengeReducer(
      activeChallengeInitialState,
      declareWinner({ id: "1", winner: "champion" })
    );
  ...
  it("should set winstreak to 1", () => {
    const actual = challengeReducer(
      activeChallengeInitialState,
      declareWinner({ id: "1", winner: "challenger" })
    );
